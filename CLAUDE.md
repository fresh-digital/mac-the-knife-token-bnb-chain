...
- **Secrets:** `.env` is per-machine and never committed (deployer key = burner wallet only).
  On a new machine, copy `.env.example` → `.env` and refill. Claude never handles the real key.
- Mainnet deployment and liquidity transactions are executed by the human, never by Claude.
